...
export default function Review() {
  ...
  const proposalTask = detail?.open_tasks.find((task) => task.task_type === "REVIEW_FIX_PROPOSAL");
  const proposed = detail?.versions.find((version) => version.id === proposalTask?.target_content_version_id);
  const auditedVersion = detail?.versions.find((version) => version.id === detail.latest_audit?.content_version_id);
  const original = auditedVersion || detail?.versions[0];
  ...
  const auditBatch = async () => {
    if (!batchId) return;
    setBusy("batch"); setMessage(null);
    try {
      const result = await api.auditBatch(batchId);
      const failures = result.results.filter((entry) => entry.status === "error");
      setMessage(failures.length
        ? { type: "err", text: `完成 ${result.audited} 条，失败 ${failures.length} 条：${failures.map((entry) => `#${entry.content_id} ${entry.error}`).join("；")}` }
        : { type: "ok", text: `批次审核已完成，共 ${result.audited} 条` });
      const nextId = await loadContents(selectedId); if (nextId) await loadDetail(nextId);
    } catch (error) { setMessage({ type: "err", text: error instanceof Error ? error.message : "批次审核失败" }); }
    finally { setBusy(""); }
  };
  ...
  const saveConfig = async (patch: Partial<Pick<Config, "reviewer" | "model">>) => {
  ...
  return <div>
    ...
    <section className="filter-bar card">
      ...
      <div className="field"><label htmlFor="review-backend">审核后端</label><select id="review-backend" value={config?.reviewer || "heuristic"} onChange={(event) => saveConfig({ reviewer: event.target.value })}><option value="heuristic">启发式规则</option><option value="oneapi">OneAPI</option><option value="ernie">文心一言</option></select></div>
    ...
    {config && !config.key_set && config.reviewer !== "heuristic" && <div className="msg err">后端未检测到 ONEAPI_KEY，在线模型审核会失败。</div>}
